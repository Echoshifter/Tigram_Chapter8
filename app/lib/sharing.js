...
if(!Alloy.Globals.FB) {
	Alloy.Globals.FB = require('facebook');//this is how to load a module
}
...
//get the app id
Alloy.Globals.FB.appid = Ti.API.Properties.getString("ti.facebook.appid");
...
exports.sharingOptions = function(_options) {
	...
	if(OS_ANDROID) {
		...
		params = {
			options : [
				'Facebook Feed', 'Facebook Photo', 'Twitter',
				'Email', 'Cancel'
			],
			cancel : 3,
			title : 'Share Photo'
...
function checkPermissions(_permissions, _callback) {
	...
	var query = "SELECT " + _permissions + " FROM permissions WHERE uids = me()";
...
function prepForFacebookShare(_callback) {
	...
	if (FB.loggedIn === false) {
		FB.addEvenListener('login', loginCB);
		FB.authorize();
	} else {
		
		//first make sure this permssion exists for user
